...
class RrtTree:
    def __init__(self) -> None:
        ...
        self.pose_to_idx_map = {}
        self.kd_pos_tree: KDTree = None
    ...
    def add_node(self, node_pose: Tuple[float, float, float], parent_idx: int) -> int:
        ...
        self.pose_to_idx_map[node_pose] = idx
        self.kd_pos_tree = KDTree([node.get_pos() for node in self.nodes])
        return idx
    ...
    def get_path_from_tree(
        ...
        """
        if goal_pose not in self.pose_to_idx_map:
            return None
        path = [goal_pose]
        goal_idx = self.pose_to_idx_map[goal_pose]
        node_on_path_idx = self.nodes[goal_idx].get_parent()
        while node_on_path_idx is not None:
            path.append(self.nodes[node_on_path_idx].get_pose())
            node_on_path_idx = self.nodes[node_on_path_idx].get_parent()
...
class RrtStarTree(RrtTree):
    ...
    def find_near_nodes(self, new_node_pos: Tuple[float, float]) -> List[int]:
        near_nodes_idx = self.kd_pos_tree.query_ball_point(
    ...
    def get_cost_of_node_pos(self, pose: Tuple[float, float, float]) -> float:
        if pose not in self.pose_to_idx_map:
            return float("inf")
        return self.nodes[self.pose_to_idx_map[pose]].get_cost()
    ...
    def update_parent_and_cost(
        ...
        original_parent_idx = self.nodes[node_idx].get_parent()
